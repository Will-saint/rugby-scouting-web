  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">{filtered.length} joueurs · Top 14 2025-2026</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="px-3 py-1.5 rounded border text-sm text-white placeholder-slate-500 outline-none focus:border-orange-500/60"
          style={{ background: "#111827", borderColor: "#1e2d42" }}
        />
        <select
          value={position}
          onChange={(e) => setPosition(e.target.value)}
          className="px-3 py-1.5 rounded border text-sm text-white outline-none"
          style={{ background: "#111827", borderColor: "#1e2d42" }}
        >
          {POSITIONS.map((p) => (
            <option key={p} value={p}>{p === "ALL" ? "Tous les postes" : POSITION_LABELS[p] || p}</option>
          ))}
        </select>
        <select
          value={team}
        ...
        >
          {teamNames.map((t) => (
            <option key={t} value={t}>{t === "ALL" ? "Toutes les équipes" : t}</option>
          ))}
        </select>
        <select
          value={tier}
        ...
        >
          {TIERS.map((t) => (
            <option key={t} value={t}>{t === "ALL" ? "Tous les tiers" : t}</option>
          ))}
        </select>
      </div>
        ...
        <table className="w-full text-sm">
          <thead style={{ background: "#0d1626" }}>
            <tr className="text-left text-slate-500 text-xs uppercase tracking-wider">
              ...
              <th className="px-4 py-3 w-32">Note</th>
          ...
          <tbody>
            {filtered.map((p) => (
              ...
              >
                <td className="px-4 py-3 text-slate-600 font-mono text-xs">{p.rank}</td>
                ...
                <td className="px-4 py-3 text-slate-400 hidden md:table-cell text-xs">{p.team}</td>
                <td className="px-4 py-3 text-slate-400 hidden lg:table-cell text-xs">{p.age ?? "—"}</td>
                <td className="px-4 py-3 text-slate-400 hidden lg:table-cell text-xs">{p.nationality ?? "—"}</td>
                <td className="px-4 py-3">
